...
<div class="row" align="center">
		...
		<table class="table" id="reply">
				...
				<form name="replyFrm">
					<input type="text" class="form-control" name="pur_inq_rep_content" placeholder="댓글을 입력하세요">
 					<input type="text" class="form-control" name="user_id" placeholder="아이디을 입력하세요">
...
$.ajax({
		...
		success: function(response) {
			for(i=0; i<response.length; i++){
				$("#reply").append(
						"<tr><td>" + response[i].pur_inq_rep_no + "</td><td>"
						+ response[i].pur_inq_rep_content + "</td><td>"
						+ response[i].user_id + "</td><td>"
						+ response[i].pur_inq_rep_date + "</td></tr>"						
						);
			}
		}
